...
class GameServer:
                ...
                with self.lock:
                    if data == "UP":
                        ...
                        self.paddles[pid] = min(
                            HEIGHT - 100, self.paddles[pid] + PADDLE_SPEED
                        )
        except:
            with self.lock:
    ...
    def ball_logic(self):
        ...
        while not self.game_over:
            with self.lock:
                ...
                if (
                    self.ball["x"] <= 40
                    and self.paddles[0] <= self.ball["y"] <= self.paddles[0] + 100
                ) or (
                    self.ball["x"] >= WIDTH - 40
                    and self.paddles[1] <= self.ball["y"] <= self.paddles[1] + 100
                ):
                    self.ball["vx"] *= -1
    ...
    def accept_players(self):
        for pid in [0, 1]:
            ...
            threading.Thread(
                target=self.handle_client, args=(pid,), daemon=True
            ).start()
